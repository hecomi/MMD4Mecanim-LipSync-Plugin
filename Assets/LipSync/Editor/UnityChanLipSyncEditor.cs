...
public sealed class UnityChanLipSyncEditor : Editor
{
	...
	{
		// Get target instance
		var lipSync = target as UnityChanLipSync;

		// Notice
		// --------------------------------------------------------------------------------
		EditorGUILayout.HelpBox("ユニティちゃんの音声を利用する際は必ず UCL をご確認下さい。 (http://unity-chan.com/download/guideline.html)", MessageType.None);
		...
		if (callibrationFoldOut_) {
			...
			if ( GUILayout.Button("Callibration", EditorStyles.miniButton) ) {
				if (aClip != null) lipSync.Callibration(aClip, UnityChanLipSync.Vowel.A);
				if (iClip != null) lipSync.Callibration(iClip, UnityChanLipSync.Vowel.I);
				if (uClip != null) lipSync.Callibration(uClip, UnityChanLipSync.Vowel.U);
				if (eClip != null) lipSync.Callibration(eClip, UnityChanLipSync.Vowel.E);
				if (oClip != null) lipSync.Callibration(oClip, UnityChanLipSync.Vowel.O);
			}
			EditorGUILayout.Separator();
			...
			if (lpcVowelFreqFoldOut_) {
				...
				EditorGUILayout.BeginHorizontal(); {
					...
					if (GUILayout.Button("Use MeiChan's default parameters", EditorStyles.miniButton)) {
						lipSync.lpcOrder  = UnityChanLipSync.meiLpcOrder;
						lipSync.sampleNum = UnityChanLipSync.meiSampleNum;
						lipSync.aCenterF1 = UnityChanLipSync.aCenterMeiF1;
						lipSync.aCenterF2 = UnityChanLipSync.aCenterMeiF2;
						lipSync.iCenterF1 = UnityChanLipSync.iCenterMeiF1;
						lipSync.iCenterF2 = UnityChanLipSync.iCenterMeiF2;
						lipSync.uCenterF1 = UnityChanLipSync.uCenterMeiF1;
						lipSync.uCenterF2 = UnityChanLipSync.uCenterMeiF2;
						lipSync.eCenterF1 = UnityChanLipSync.eCenterMeiF1;
						lipSync.eCenterF2 = UnityChanLipSync.eCenterMeiF2;
						lipSync.oCenterF1 = UnityChanLipSync.oCenterMeiF1;
						lipSync.oCenterF2 = UnityChanLipSync.oCenterMeiF2;
					}
